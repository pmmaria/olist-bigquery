...
WITH category_sales AS (
    SELECT
        ...
        RANK() OVER (ORDER BY SUM(order_value_total) DESC) AS sales_rank
    FROM
        olist.cleaned_order_items
        JOIN olist.cleaned_products USING (product_id)
    GROUP BY
...
category_orders AS (
    SELECT
        product_category,
        COUNT(order_id) AS total_orders,
        RANK() OVER (ORDER BY COUNT(order_id) DESC) AS orders_rank
    FROM
        olist.cleaned_order_items
        JOIN olist.cleaned_products USING (product_id)
    GROUP BY
...
FULL OUTER JOIN
    category_orders co
    ON cs.product_category = co.product_category
WHERE
    cs.sales_rank <= 10 OR co.orders_rank <= 10
ORDER BY
